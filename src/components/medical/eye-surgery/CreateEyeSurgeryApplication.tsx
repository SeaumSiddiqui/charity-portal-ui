...
export const CreateEyeSurgeryApplication: React.FC = () => {
  const navigate = useNavigate();
  const { showToast } = useToast();
  const [saving, setSaving] = useState(false);
  const [hasUnsavedChanges, setHasUnsavedChanges] = useState(false);
  const [initialValues] = useState<MedicalApplication>(
    createEmptyMedicalApplication(SurgeryType.EYE_SURGERY)
  );

  const handleSubmit = async (values: MedicalApplication, isSubmit = false) => {
    setSaving(true);
    try {
      const savedApplication = await eyeSurgeryService.createApplication(values);

      showToast(
        'success',
        isSubmit ? 'Application Submitted' : 'Draft Saved',
        isSubmit
          ? 'Eye surgery application has been submitted successfully.'
      ...
      if (savedApplication.id) {
        navigate(`/medical/eye-surgery/${savedApplication.id}`);
      }
    } catch (error: any) {
  ...
  return (
    <MedicalApplicationForm
      ...
      onExit={() => navigate('/medical/eye-surgery')}
    />
  );
};
